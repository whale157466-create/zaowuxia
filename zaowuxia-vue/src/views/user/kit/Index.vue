...
<template>
  <div v-loading="loading" style="max-width: 640px; margin: 0 auto;">
    ...
    <div v-for="(group, label) in { '📦 材料清单': materials, '🔨 配套工具': tools }" :key="label" style="margin-top: 20px;">
      ...
      <div v-for="item in group" :key="item.skuId" style="background: var(--zao-surface); border-radius: var(--zao-radius); padding: 12px; margin-bottom: 8px; display: flex; align-items: center; gap: 12px;" :style="{ opacity: item.stock === 0 ? 0.5 : 1 }">
        ...
        <template v-if="item.stock === 0"><span style="font-size: 13px; color: var(--zao-danger);">缺货</span></template><!-- E022 -->
        <template v-else>
    ...
    <!-- M5-5: 汇总 + M5-6: 加入购物车 -->
    <div class="flex-between" style="position: sticky; bottom: 0; margin-top: 16px; background: var(--zao-surface); border-radius: var(--zao-radius); padding: 16px; box-shadow: var(--zao-shadow-up);">
